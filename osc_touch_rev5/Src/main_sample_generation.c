...
void prepareCV_FM_Morph(audioRateInputs * audioInputs, controlRateInputs *controlInputs, q31_t *incrementValues, q31_t *phaseModValues, q31_t * morphValues, q31_t *pwmValues) {
	...
	q31_t frequencyMultiplier = (((fix16_mul(fix16_mul(expoTable[controlInputs->knob1Value] >> 3, expoTable[controlInputs->knob2Value >> 3]), expoTable[4095 - controlInputs->cv1Value] >> 2) * touchkeyValues.button1Value) >> 9) * touchkeyValues.button2Value) >> 8;
	arm_offset_q31(audioInputs->xCV, -1024 - touchkeyValues.button3Value, incrementValues, BUFFER_SIZE);
