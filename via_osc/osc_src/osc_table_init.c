...
void loadSampleArray(Family * family) {
	...
	for (int i = 0; i < family->familySize; i++) {
		...
		fullTableHoldArray[i][0] = *(*(family->releaseFamily + i) + 0) >> 3;
		fullTableHoldArray[i][1] = *(*(family->releaseFamily + i) + 0) >> 3;
		...
		for (int j = 0;j < numSamples; j++) {
			fullTableHoldArray[i][2 + j] = *(*(family->attackFamily + i) + j) >> 3;
			fullTableHoldArray[i][2 + numSamples + j] = *(*(family->releaseFamily + i) + family->tableLength - j) >> 3;
		}
		//pad out the "first two" samples from attack
		fullTableHoldArray[i][(numSamples << 1) + 2] = *(*(family->attackFamily + i) + 0) >> 3;
		fullTableHoldArray[i][(numSamples << 1) + 3] = *(*(family->attackFamily + i) + 0) >> 3;
		fullTableHoldArray[i][(numSamples << 1) + 4] = *(*(family->attackFamily + i) + 0) >> 3;
	}

	//for each table in the family
	for (int i = 0; i < family->familySize - 1; i++) {

		for (int j = 0; j < (numSamples << 1) + 5; j++) {
			fullTableHoldArray[i][j] |= (fullTableHoldArray[i + 1][j] - fullTableHoldArray[i][j]) << 16;
		}
