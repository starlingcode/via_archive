...
static inline int getSampleQuinticSpline(int, uint32_t, logicHandler *);
...
void getSamplesDifferential(q31_t * phaseArray, int PWM, q31_t * morphArray, q31_t * samplesA, q31_t * samplesB, logicHandler * deltaArray) {

	for (int i = 0; i < BUFFER_SIZE; i++) {
		int sample = getSampleQuinticSpline(phaseArray[i], __USAT(morphArray[i], 12), &deltaArray[i]) - 2048;
		if (sample  > 0) {
			samplesA[i] = sample << 1;
			samplesB[i] = 0;
		} else {
			samplesA[i] = 0;
			samplesB[i] = abs(sample) << 1;
		}
	}
...
static inline int getSampleQuinticSpline(int phase, uint32_t morph, logicHandler * delta) {
