...
void ui_button2Menu(int sig)
{
	...
	case SENSOR_EVENT_SIG:
		if (BUTTON2SENSOR == RELEASED){
			if(UI_TIMER_READ < 3000){
				button2Mode = (button2Mode + 1) % numButton2Modes;
				modeStateBuffer = (modeStateBuffer & ~(BUTTON2_MASK)) | (button5Mode << BUTTON2_SHIFT);
				handleButton2ModeChange(button2Mode);
				uiClearLEDs();
...
void ui_button5Menu(int sig)
{
	...
	case SENSOR_EVENT_SIG:
		if (BUTTON5SENSOR == RELEASED){
			if(UI_TIMER_READ < 3000){
				button5Mode = (button5Mode + 1) % numButton5Modes;
				modeStateBuffer = (modeStateBuffer & ~(BUTTON5_MASK)) | (button5Mode << BUTTON5_SHIFT);
				handleButton6ModeChange(button5Mode);
				uiClearLEDs();
