...
static inline int getSampleQuinticSplineX(int, uint32_t);
static inline int getSampleQuinticSplineY(int, uint32_t);
...
void foldBuffer(q31_t * input, q31_t offset, q31_t * output) {
	...
	arm_scale_q31(input, 1 << 31, 6, output, BUFFER_SIZE);
...
static inline int getSampleProduct(int xIndex, int yIndex, int zIndex) {
	...
	int xSample = getSampleQuinticSplineX(xIndex << 10, zIndex >> 3);
	int ySample = getSampleQuinticSplineY(yIndex << 10, zIndex >> 3);
...
static inline int getSamplePM(int xIndex, int yIndex, int zIndex) {
	...
	int xSample = getSampleQuinticSplineX(xIndex << 10, zIndex);
	int ySample = getSampleQuinticSplineY(((yIndex << 10) + (xSample << 4)) & ((1<<25) - 1), zIndex);
...
static inline int getSampleQuinticSplineX(int phase, uint32_t morph) {
    ...
    */
	...
	leftIndex = &fullTableHoldArrayX[LnFamily][LnSample];
	rightIndex = &fullTableHoldArrayX[LnFamily + 1][LnSample];
...
static inline int getSampleQuinticSplineY(int phase, uint32_t morph) {
    ...
    */
	...
	leftIndex = &fullTableHoldArrayY[LnFamily][LnSample];
	rightIndex = &fullTableHoldArrayY[LnFamily + 1][LnSample];
